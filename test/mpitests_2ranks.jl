...
@testset "MPI Getindex for Function n=$n" for n in ( (11,10), (5,4), (7,2), (6,5,4) )
    N = length(n)
    ...
    copy!(data(f), arr)
    df = convert(Array, data(f))
    if prod(size(df)) > 0
        if N == 2
            @test df[1:div(n[1],2),:] ≈ arr[1:div(n[1],2),:]
        else
            @test df[1:div(n[1],2),2:2,:] ≈ arr[1:div(n[1],2),2:2,:]
        end
    end
end

@testset "MPI Getindex for TimeFunction n=$n" for n in ( (11,10), (5,4), (7,2), (6,5,4) )
    N = length(n)
    nt = 5
    rnk = MPI.Comm_rank(MPI.COMM_WORLD)
    grid = Grid(shape=n, dtype=Float32)
    ...
    if prod(size(df)) > 0
        if N == 2
            @test df[1:div(n[1],2),:,1:1] ≈ arr[1:div(n[1],2),:,1:1]
        else
            @test df[1:div(n[1],2),2:2,:,1:1] ≈ arr[1:div(n[1],2),2:2,:,1:1]
        end
    end
end
...
@testset "MPI Getindex for SparseFunction n=$n npoint=$npoint" for n in ( (5,4),(4,5,6) ), npoint in (1,5,10)
    ...
    if prod(size(df)) > 0
        if npoint > 1
            @test df[1:div(npoint,2)] ≈ arr[1:div(npoint,2)]
        else
            @test df[1] == arr[1]
        end
    end
