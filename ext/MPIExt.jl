...
using Devito.PythonCall: pynew, pyisnull, pycopy!
...
function DevitoMPIArray{T,N}(o, idxs, decomp::D, topo) where {T,N,D}
    p = unsafe_wrap(Array{T,N}, Ptr{T}(pyconvert(UInt, o.__array_interface__["data"][0])), length.(idxs); own=false)
...
function localindices(x::DiscreteFunction{T,N,DevitoMPITrue}) where {T,N}
    if pyhasattr(x.o, "local_indices") && !pyis(x.o.local_indices, pybuiltins.None)
